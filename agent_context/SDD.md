...
**`site_metadata`** — registro único (`id` fixo): `title`, `description`, `og_image_url` (`text`, URL pública pronta para `<meta property="og:image">`, ou `null`), `updated_at`, `updated_by`. Sem coluna de URL canônica: o PRD não exige controle de URL canônica pelo painel, e uma coluna sem uso real seria uma pendência de modelagem desde o dia um.
