...
### Bucket MinIO (`images`) — migração em andamento

O serviço `minio-init` (`docker-compose.yml`, tarefa `ajustes/migracao-mysql-infra-compose`) provisiona o mesmo bucket `images` no MinIO auto-hospedado que vai substituir o Storage do Supabase acima (ver `agent_context/CHANGELOG.md`, decisão de 2026-09-21). Depois de criar o bucket (`mc mb`), o entrypoint (`docker/minio/init-bucket.sh`) aplica uma policy JSON própria via `mc anonymous set-json`, contendo só a ação `s3:GetObject` sobre `arn:aws:s3:::images/*` — concede leitura pública **só de objeto** (um `GET` anônimo de uma URL de objeto específica funciona), sem conceder `s3:ListBucket` (listagem do bucket) nem escrita anônima, o equivalente funcional à policy `images_public_read` do Supabase original (`storage.buckets.public = true` + policy de `select`, ver `supabase/migrations/20260908200805_enable_rls_and_storage.sql`). **Não** usamos a policy canônica `mc anonymous set download`: confirmado empiricamente (`mc anonymous get-json`) que ela concede também `s3:ListBucket`/`s3:GetBucketLocation` no bucket inteiro, o que exporia a lista completa de arquivos — além do que o critério de "pronto" da tarefa permite. Sem nenhuma dessas policies, o MinIO nega `GET` anônimo por padrão e a `public_url` gravada em `media_assets` por `MinioMediaAssetsRepository` teria a forma de uma URL pública sem ser de fato acessível (achado e correção registrados em `agent_context/PLAN.md`, tarefa `ajustes/migracao-mysql-minio-bucket-leitura-publica`).

**Endpoint alcançável pelo navegador.** O serviço `minio` publica sua porta da API S3 (9000, não a do console 9001) para o host (`docker-compose.yml`) e o serviço `api` assina a URL de upload/monta `public_url` com `MINIO_ENDPOINT` apontando para esse mesmo endpoint publicado — nunca para o hostname interno `minio:9000`, resolvível só dentro da rede do compose e por isso inalcançável por qualquer navegador real. Default de desenvolvimento: `http://localhost:9000`; em homologação/produção, `MINIO_ENDPOINT` precisa ser sobrescrita com o domínio/IP público real (achado e correção registrados em `agent_context/PLAN.md`, tarefa `ajustes/migracao-mysql-minio-endpoint-publico`). Documentação completa da migração para MySQL/MinIO fica consolidada na tarefa `ajustes/migracao-mysql-documentacao`.
