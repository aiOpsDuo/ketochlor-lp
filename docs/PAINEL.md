...
Diferente da simplificação aceita originalmente em `painel/tela-metadados` (`ogImageMediaId` como texto livre, sem upload nenhum — corrigida depois pela tarefa `ajustes/corrige-imagem-metadados`, ver seção "Metadados da página" abaixo), aqui o operador realmente envia o arquivo pelo painel. O campo "URL da imagem" continua editável ABAIXO do botão de upload, não como substituto dele, por dois motivos:
