...
const CLASSE_CAMPO_BASE =
  'w-full rounded-lg border bg-white px-3 py-2 text-sm text-navy placeholder:text-slate-400 focus:outline-none focus:ring-2'
 ...
 * há disputa de especificidade para perder.
 */
const CLASSE_CAMPO_NORMAL = 'border-slate-300 focus:border-slate-500 focus:ring-slate-500/30'
const CLASSE_CAMPO_INVALIDO = 'border-red-400 focus:border-red-500 focus:ring-red-500/30'
...
/** Rótulo de campo. */
export const CLASSE_ROTULO = 'text-sm font-medium text-navy'

/** Texto de ajuda abaixo de um campo. */
export const CLASSE_AJUDA = 'text-xs text-slate-500'

/** Mensagem de erro de um campo. */
export const CLASSE_ERRO_DE_CAMPO = 'text-sm text-red-600'

export type VarianteDeBotao = 'primario' | 'secundario' | 'perigo'
export type TamanhoDeBotao = 'medio' | 'pequeno'

const BASE_BOTAO =
  'inline-flex items-center justify-center gap-1.5 rounded-lg font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500/40 disabled:cursor-not-allowed disabled:opacity-50'
