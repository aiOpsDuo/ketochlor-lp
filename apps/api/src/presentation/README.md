...
`MediaAdminController` (`/api/admin/media`) — uma única rota, `POST /upload-url`: valida o corpo via `validarSolicitacaoUpload` do Domínio (`originalFilename` não vazio, `mimeType` começando com `"image/"`, `422` se inválido) e devolve a credencial de upload direto ao Storage (`signedUrl`, `token`) mais o `mediaAssetId` reservado — nenhum byte de arquivo passa pela API (SDD § Decisões técnicas e trade-offs). `MEDIA_ASSETS_REPOSITORY` ligado a `SupabaseMediaAssetsRepository` (Infraestrutura, já existente desde `api/infra-supabase-adapters`) em `media.module.ts`. A confirmação do registro em `media_assets` depois que o upload termina (`MediaAssetsRepository.criar`) não tem rota própria nesta tarefa — ver nota de decisão em `media-admin.controller.ts`.
