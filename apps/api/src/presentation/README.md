...
`AuthGuard` (`CanActivate`) protege toda rota sob `/api/admin/*`, verificando o JWT próprio da aplicação via a porta `VerificadorToken` (Domínio), ligada em `auth.module.ts` à implementação `AppJwtTokenVerificador` (Infraestrutura, HS256, `AUTH_JWT_SECRET`). Registrado GLOBALMENTE via `APP_GUARD` — decide sozinho, pelo caminho da requisição, se exige token (ver comentário de decisão em `auth/auth.guard.ts`). Os módulos `metadata`, `media`, `leads` e `operators` importam `AuthModule` e reaproveitam o mesmo guard; nenhum precisa recriar autenticação ou decorar seus controllers manualmente, desde que suas rotas fiquem sob `/api/admin`. `POST /api/auth/login` (`AuthLoginModule`) emite o token a partir de e-mail/senha conferidos contra a tabela `operators` (MySQL).
...
Casos de uso em `apps/api/src/application/content/*.use-case.ts`; nenhuma regra de negócio nos controllers, só tradução HTTP↔caso de uso. `CONTENT_SECTIONS_REPOSITORY` (token de injeção, definido na Aplicação) é ligado a `MySqlContentSectionsRepository` em `content.module.ts`, mesmo padrão de `VERIFICADOR_TOKEN`/`auth.module.ts`.

## `media` (tarefa `api/modulo-media`)

`MediaAdminController` (`/api/admin/media`) — uma única rota, `POST /upload-url`: valida o corpo via `validarSolicitacaoUpload` do Domínio (`originalFilename` não vazio, `mimeType` começando com `"image/"`, `422` se inválido) e devolve a credencial de upload direto ao MinIO (`signedUrl`) mais o `mediaAssetId` reservado — nenhum byte de arquivo passa pela API (SDD § Decisões técnicas e trade-offs). `MEDIA_ASSETS_REPOSITORY` ligado a `MinioMediaAssetsRepository` (Infraestrutura) em `media.module.ts`. A confirmação do registro em `media_assets` depois que o upload termina (`MediaAssetsRepository.criar`) não tem rota própria — ver nota de decisão em `media-admin.controller.ts`.
