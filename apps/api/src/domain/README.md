...
`metadata/validar-site-metadata.ts` (tarefa `api/modulo-metadata`) segue o mesmo padrão de `validarLead` para `site_metadata` (sem esquema Zod compartilhado). `media/validar-solicitacao-upload.ts` (tarefa `api/modulo-media`) faz o mesmo para o corpo de `POST /api/admin/media/upload-url`: `originalFilename` obrigatório e `mimeType` obrigatório começando com `"image/"` — regra de negócio de "só imagem, sem vídeo nesta versão" (PRD § Fora de escopo), por isso vive aqui e não numa checagem de forma de DTO na Apresentação.

`shared/resultado-validacao.ts` define o tipo `ResultadoValidacao<T>` comum a todas as validações acima, para não duplicar a mesma forma de "sucesso ou lista de erros".
